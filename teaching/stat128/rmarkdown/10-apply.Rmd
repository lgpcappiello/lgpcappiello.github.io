...
Consider the built-in data set `iris`. If we use the `as.list()` function, each column will be converted into an element of a list.
```{r}
head(iris)
str(as.list(iris))
```

So if we use `lapply()` it will iterate over the *columns*. We can find all values within a variable that are greater than the variable mean (for columns 1-4, the numeric variables).
